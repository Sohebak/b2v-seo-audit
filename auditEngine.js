...
class SEOAuditor {
  ...
  async runAudit() {
    ...
    try {
      ...
      if (process.env.GOOGLE_PAGESPEED_API_KEY && process.env.GOOGLE_PAGESPEED_API_KEY !== 'your-api-key-here') {
  ...
  async checkSSL() {
    try {
      ...
      this.results.checks.ssl = {
        ...
        recommendation: 'Ensure your website has a valid SSL certificate installed.'
      };
    }
  }
  ...
  checkMetaTags($) {
    ...
    if (title.length > 0) {
      ...
      if (title.length < 30 || title.length > 60) {
        ...
        recommendations.push('Adjust title length to 30-60 characters for better display in search results');
      }
    } else {
    ...
    if (metaDesc.length > 0) {
      ...
      if (metaDesc.length < 120 || metaDesc.length > 160) {
        ...
        recommendations.push('Optimize meta description length for better SERP display');
      }
    } else {
    ...
    this.results.checks.metaTags = {
      ...
      score: score,
      message: issues.length > 0 ? issues.join(' | ') : 'Meta tags are properly configured',
      recommendation: recommendations.length > 0 ? recommendations.join('. ') : null,
      details: { title, metaDesc, titleLength: title.length, descLength: metaDesc.length }
    };
  }
  ...
  checkHeadings($) {
    ...
    this.results.checks.headings = {
      ...
      score: score,
      message: message,
      recommendation: h1Count !== 1 ? 'Use exactly one H1 tag per page for optimal SEO' : null,
  ...
  checkImages($) {
    ...
    this.results.checks.images = {
      ...
      recommendation: score < 100 ? 'Add descriptive alt text to all images for better accessibility and SEO' : null,
      details: { total: totalImages, withAlt: imagesWithAlt, missing: totalImages - imagesWithAlt }
    };
  }
  ...
  checkPageSpeed(loadTime) {
    ...
    this.results.checks.pageSpeed = {
      ...
      recommendation: score < 100 ? 'Optimize images, enable caching, and minify CSS/JS to improve load time' : null,
  ...
  async checkPageSpeedWithGoogle() {
    try {
      ...
      this.results.checks.pageSpeed = {
        ...
        recommendation: score < 80 ? 'Improve Core Web Vitals: LCP, FID, and CLS for better performance' : null,
        details: {
  ...
  async checkMobileResponsive(page) {
    try {
      ...
      this.results.checks.mobile = {
        ...
        message: isMobileFriendly ? 'Website is mobile-friendly!' : 'No mobile viewport detected',
        recommendation: isMobileFriendly ? null : 'Add viewport meta tag for mobile responsiveness',
  ...
  checkSocialTags($) {
    ...
    this.results.checks.socialTags = {
      ...
      status: score >= 80 ? 'pass' : score >= 50 ? 'warning' : 'fail',
      score: score,
      message: score >= 80 ? 'Social media tags are properly configured!' : 'Missing some social media tags',
      recommendation: score < 80 ? 'Add Open Graph and Twitter Card tags for better social sharing' : null,
  ...
  checkContent($) {
    ...
    this.results.checks.content = {
      ...
      score: score,
      message: message,
      recommendation: wordCount < 300 ? 'Add more high-quality, relevant content (aim for 300+ words)' : null,
      details: { wordCount }
    };
  }
  ...
  checkLinks($) {
    ...
    this.results.checks.links = {
      ...
      recommendation: null,
      details: { total: totalLinks, internal: internalLinks, external: externalLinks }
    };
  }
  ...
  async checkRobotsAndSitemap() {
    try {
      ...
      this.results.checks.robotsAndSitemap = {
        ...
        recommendation: score < 100 ? 'Add missing robots.txt and/or sitemap.xml files' : null,
