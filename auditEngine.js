...
class SEOAuditor {
  ...
  async runAudit() {
    ...
    try {
      ...
      // Get the ACTUAL loaded URL (after redirects)
      const actualUrl = page.url();
      console.log(`Actual loaded URL: ${actualUrl}`);

      const html = await page.content();
      const $ = cheerio.load(html);

      // Check SSL based on the ACTUAL URL loaded
      this.checkSSL(actualUrl);
      this.checkMetaTags($);
      this.checkHeadings($);
      this.checkImages($);
  ...
  checkSSL(actualUrl) {
    try {
      // Check the ACTUAL loaded URL (after any redirects)
      const urlToCheck = actualUrl || this.url;
      const isSSL = urlToCheck.startsWith("https://");
